#!/usr/bin/env python3
"""
SMA 20 Trading Bot - Trend Direction & Dynamic Support/Resistance
================================================================
Full analysis engine for Spot and Futures (Long/Short) trading signals.

Features:
  - SMA 20 trend direction analysis
...
class IndicatorValues:
    sma20: float = 0.0
    sma20_prev: float = 0.0
    sma20_prev2: float = 0.0
    ema12: float = 0.0
    ema26: float = 0.0
    macd_line: float = 0.0
...
def load_candles(file_path: str) -> List[Candle]:
    ...
    if len(candles) < 30:
        print(f"[ERROR] Need at least 30 candles, got {len(candles)}")
        sys.exit(1)
...
class TrendAnalyzer:
    ...
    def analyze_trend_direction(candles: List[Candle], indicators: IndicatorValues,
                                 sma20_values: List[float]) -> TrendDirection:
        ...
        sma_slope = indicators.sma20 - indicators.sma20_prev
        prev_slope = indicators.sma20_prev - indicators.sma20_prev2
        ...
        upper_cross = False
        lower_cross = False
        for i in range(max(1, len(sma20_values) - 5), len(sma20_values)):
            if candles[i].close > sma20_values[i] and candles[i-1].close <= sma20_values[i-1]:
                upper_cross = True
            if candles[i].close < sma20_values[i] and candles[i-1].close >= sma20_values[i-1]:
                lower_cross = True
    ...
    def analyze_market_phase(candles: List[Candle], indicators: IndicatorValues,
                             sma20_values: List[float]) -> MarketPhase:
...
class SupportResistanceDetector:
    ...
    def calculate_dynamic_levels(candles: List[Candle],
        ...
        if indicators.sma20 > 0:
            if indicators.sma20 < current_price:
                supports.append(indicators.sma20)
            else:
                resistances.append(indicators.sma20)
...
class SignalGenerator:
    ...
    def calculate_all_indicators(self):
        ...
        self.sma20_values = self.ti.sma(self.closes, 20)
        ...
        self.indicators.sma20 = self.sma20_values[i]
        self.indicators.sma20_prev = self.sma20_values[i-1] if i > 0 else self.sma20_values[i]
        self.indicators.sma20_prev2 = self.sma20_values[i-2] if i > 1 else self.sma20_values[i]
        ...
        print(f"[INFO] SMA20: {self.indicators.sma20:.2f}")
    ...
    def generate_signal(self) -> SignalResult:
        ...
        print("\n" + "=" * 70)
        print("  SMA 20 TRADING BOT - TREND & DYNAMIC SUPPORT/RESISTANCE ANALYSIS")
        print("=" * 70)
        ...
        trend_direction = self.ta.analyze_trend_direction(
            self.candles, self.indicators, self.sma20_values
        )
        ...
        self._evaluate_sma20_position(current_price, bull_score, bear_score,
                                       reasons_bull, reasons_bear)
        bull_score, bear_score, reasons_bull, reasons_bear = self._evaluate_sma20_position(
            current_price, bull_score, bear_score, reasons_bull, reasons_bear
        )

        bull_score, bear_score, reasons_bull, reasons_bear = self._evaluate_sma20_slope(
        ...
        sma_distance = abs(current_price - self.indicators.sma20) / self.indicators.sma20 * 100
        if sma_distance > 8:
            warnings.append(f"Price {sma_distance:.1f}% away from SMA20 - overextension risk")
        if sma_distance > 15:
            warnings.append("Extreme deviation from SMA20 - pullback likely")
        ...
        result.indicators_summary = {
            'SMA20': f"{self.indicators.sma20:.2f}",
            'SMA20 Slope': f"{self.indicators.sma20 - self.indicators.sma20_prev:.2f}",
    ...
    def _evaluate_sma20_position(self, price, bull, bear, reasons_b, reasons_s):
        """Evaluate price position relative to SMA20."""
        if price > self.indicators.sma20:
            bull += 3
            reasons_b.append(f"Price ({price:.2f}) is ABOVE SMA20 ({self.indicators.sma20:.2f}) - bullish bias")
        else:
            bear += 3
            reasons_s.append(f"Price ({price:.2f}) is BELOW SMA20 ({self.indicators.sma20:.2f}) - bearish bias")
        return bull, bear, reasons_b, reasons_s

    def _evaluate_sma20_slope(self, bull, bear, reasons_b, reasons_s):
        """Evaluate SMA20 slope for trend direction."""
        slope = self.indicators.sma20 - self.indicators.sma20_prev
        prev_slope = self.indicators.sma20_prev - self.indicators.sma20_prev2
        if slope > 0 and prev_slope > 0:
            bull += 2
            reasons_b.append(f"SMA20 slope is POSITIVE ({slope:.2f}) - consistent uptrend")
        elif slope < 0 and prev_slope < 0:
            bear += 2
            reasons_s.append(f"SMA20 slope is NEGATIVE ({slope:.2f}) - consistent downtrend")
        elif slope > 0:
            bull += 1
            reasons_b.append(f"SMA20 slope turning positive ({slope:.2f}) - potential reversal")
        else:
            bear += 1
            reasons_s.append(f"SMA20 slope turning negative ({slope:.2f}) - potential reversal")
        return bull, bear, reasons_b, reasons_s
...
class ReportGenerator:
    ...
    def generate_report(result: SignalResult, candles: List[Candle],
        ...
        lines.append("")
        lines.append("=" * 70)
        lines.append("         SMA 20 TRADING BOT - COMPLETE ANALYSIS REPORT")
        lines.append("=" * 70)
        lines.append("")
...
def main():
    """Main entry point for the SMA 20 Trading Bot."""
    print("\n" + "=" * 70)
    print("  SMA 20 TRADING BOT v2.0")
    ...
    report_filename = f"sma20_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    ...
    def analyze_timeframes(candles: List[Candle]) -> Dict[str, Dict[str, Any]]:
        ...
        for tf_name, multiplier in timeframes.items():
            ...
            if hist_val > hist_prev:
                ...
                score -= 1
            sma_slope = sma20[i] - sma20[i-1] if i > 0 else 0
            if sma_slope > 0:
                score += 1
            else:
                score -= 1
...
class EnhancedSignalGenerator:
    ...
    def run_full_analysis(self) -> Dict[str, Any]:
        ...
        sma20_prev = sma20[i-1] if i > 0 else sma20[i]
        sma20_prev2 = sma20[i-2] if i > 1 else sma20[i]
        ...
        self.indicators = IndicatorValues(
            sma20=sma20[i], sma20_prev=sma20_prev, sma20_prev2=sma20_prev2,
    ...
    def generate_enhanced_section(enhanced_data: Dict[str, Any],
        ...
        for tf, data in mtf.items():
            ...
            lines.append(f"  {tf:4s}: {data['trend']:10s} {trend_icon} "
                        f"SMA20={data.get('sma20', 0):.2f} RSI={data.get('rsi', 50):.1f} "
...
def main():
    """Main entry point for the SMA 20 Trading Bot with enhanced analysis."""
    print("\n" + "=" * 70)
    print("  SMA 20 TRADING BOT v2.0 - ENHANCED EDITION")
    print("  Trend Direction & Dynamic Support/Resistance Analyzer")
    ...
    report_filename = f"sma20_full_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    report_path = os.path.join(os.path.dirname(__file__), report_filename)
    try:
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write(full_report)
...
class BacktestEngine:
    ...
    def run_sma20_backtest(self, initial_capital: float = 10000.0,
                            commission_pct: float = 0.1) -> Dict[str, Any]:
        """Backtest SMA 20 crossover strategy."""
        print("[BACKTEST] Running SMA 20 backtest...")
        ti = TechnicalIndicators()
        sma20 = ti.sma(self.closes, 20)
    ...
    def format_backtest_report(self, bt_result: Dict[str, Any]) -> str:
        ...
        lines.append("")
        lines.append("=" * 70)
        lines.append("         BACKTEST RESULTS - SMA 20 STRATEGY")
        lines.append("=" * 70)
        lines.append("")
...
class AlertSystem:
    ...
    def check_alerts(candles: List[Candle], indicators: IndicatorValues,
        ...
        prev_above_sma = candles[-2].close > indicators.sma20_prev if len(candles) > 1 else False
        curr_above_sma = current_price > indicators.sma20
        if not prev_above_sma and curr_above_sma:
            alerts.append({
                'type': 'SMA_CROSS',
                'priority': 'HIGH',
                'message': f"Price crossed above SMA20 ({indicators.sma20:.2f})",
            ...
            alerts.append({
                'type': 'SMA_CROSS',
                'priority': 'HIGH',
                'message': f"Price crossed below SMA20 ({indicators.sma20:.2f})",
...
class BeautifulPDFGenerator:
    ...
    def _page_cover(self, pdf, result):
        ...
        sig_color = C["green"] if "BUY" in result.signal_type.value else C["red"] if "SELL" in result.signal_type.value else C["orange"]
        _text(ax, 0.5, 0.84, "SMA 20", fs=42, color=C["white"], ha="center", fw="bold")
        ...
        _text(ax, 0.5, 0.04, "SMA 20 Trading Bot v2.0  ·  Generated by Toobit Engine",
    ...
    def _page_risk(self, pdf, _=None):
        ...
        for title, color, items in lines:
            ...
            for item in items:
                ...
                y -= 0.035
            y -= 0.03
        _text(ax, 0.5, 0.04, f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M UTC')}  |  SMA 20 Bot v2.0",
              fs=8, color=C["text_dim"], ha="center")
        pdf.savefig(fig, facecolor=C["bg"])
        plt.close(fig)
...
class ChartGenerator:
    ...
    def _create_candlestick_panel(self, ax, indices, opens, highs, lows, closes,
        ...
        if sma20_vals is not None:
            valid_sma = [(i, v) for i, v in zip(indices, sma20_vals) if v > 0]
            if valid_sma:
                si, sv = zip(*valid_sma)
                ax.plot(si, sv, color=COLORS["cyan"], linewidth=1.8, label="SMA 20", alpha=0.9)
    ...
    def _create_volume_panel(self, ax, indices, volumes, opens, closes):
        ...
        if len(volumes) > 20:
            ...
            if valid_vol:
                vi, vv = zip(*valid_vol)
                ax.plot(vi, vv, color=COLORS["blue"], linewidth=1.0, label="Vol SMA20")
        ax.legend(loc="upper left", fontsize=7, facecolor=COLORS["bg2"],
    ...
    def generate_full_chart(self, indicators: IndicatorValues,
        ...
        fig.text(0.5, 0.98, f"SMA 20 TRADING BOT  |  {result.signal_type.value}  |  "
                 f"Confidence: {result.confidence:.0f}%",
...
class PDFReportGenerator:
    ...
    def _add_cover_page(self, pdf, result: SignalResult):
        ...
        ax.set_facecolor(COLORS["bg"])
        ax.axis("off")
        ax.text(0.5, 0.88, "SMA 20 TRADING BOT", fontsize=28, fontweight="bold",
        ...
        ax.text(0.5, 0.03, "Generated by SMA 20 Trading Bot v2.0",
...
def main():
    """Main entry point for the SMA 20 Trading Bot - Complete Edition."""
    print("\n" + "=" * 70)
    print("  SMA 20 TRADING BOT v2.0 - COMPLETE EDITION")
    print("  Trend Direction & Dynamic Support/Resistance Analyzer")
    ...
    bt_result = backtest.run_sma20_backtest()
    ...
    report_filename = f"sma20_complete_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    report_path = os.path.join(os.path.dirname(__file__), report_filename)
    try:
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write(full_report)
    ...
    try:
        ...
        json_filename = f"sma20_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    png_path = os.path.join(os.path.dirname(__file__), f"sma20_analysis_{timestamp_str}.png")
    try:
    ...
    pdf_path = os.path.join(os.path.dirname(__file__), f"sma20_report_{timestamp_str}.pdf")
    try:
    ...
    print(f"  JSON Data:         sma20_data_{timestamp_str}.json")
    print("=" * 70)
